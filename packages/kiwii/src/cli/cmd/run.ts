...
//   2. Interactive local (`kiwii --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`kiwii --mini --attach`): connects to a running
//      kiwii server and runs interactive mode against it.
//
...
import { createKiwiiClient, type KiwiiClient, type ToolPart } from "@kiwii/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<KiwiiClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run kiwii with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running kiwii server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to KIWII_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to KIWII_SERVER_USERNAME or 'kiwii')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: KiwiiClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function current(sdk: KiwiiClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: KiwiiClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function execute(sdk: KiwiiClient) {
        ...
        async function loop(client: KiwiiClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createKiwiiClient({
        baseUrl: "http://kiwii.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
