...
import { errorMessage } from "@kiwii/tui/util/error"
import { withTimeout } from "@/util/timeout"
import { withNetworkOptions, resolveNetworkOptionsNoConfig, hasArg } from "@/cli/network"
import { Filesystem } from "@/util/filesystem"
import type { GlobalEvent } from "@kiwii/sdk/v2"
import type { EventSource } from "@kiwii/tui/context/sdk"
import { writeHeapSnapshot } from "v8"
import { ServerAuth } from "@/server/auth"
import { validateSession } from "../tui/validate-session"
import { win32InstallCtrlCGuard } from "@kiwii/tui/terminal-win32"

declare global {
  const KIWII_WORKER_PATH: string
}
...
async function target() {
  if (typeof KIWII_WORKER_PATH !== "undefined") return KIWII_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start kiwii tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start kiwii in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
