...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall kiwii and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g kiwii-ai",
      pnpm: "pnpm uninstall -g kiwii-ai",
      bun: "bun remove -g kiwii-ai",
      yarn: "yarn global remove kiwii-ai",
      brew: "brew uninstall kiwii",
      choco: "choco uninstall kiwii",
      scoop: "scoop uninstall kiwii",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "kiwii", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Kiwii!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# kiwii") || content.includes(".kiwii/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".kiwii/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".kiwii/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".kiwii"))
    ) {
      continue
    }
