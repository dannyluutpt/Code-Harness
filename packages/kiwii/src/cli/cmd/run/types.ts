// Shared type vocabulary for the direct interactive mode (`kiwii --mini`).
//
...
import type { KiwiiClient, PermissionRequest, QuestionRequest, ToolPart } from "@kiwii/sdk/v2"
import type { TuiConfig } from "@kiwii/tui/config"
...
type PromptModel = Parameters<KiwiiClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<KiwiiClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<KiwiiClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<KiwiiClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<KiwiiClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<KiwiiClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<KiwiiClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<KiwiiClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<KiwiiClient["question"]["reject"]>[0]
