...
export function RunCommandMenuBody(props: {
  ...
  const entries = createMemo<CommandEntry[]>(() => {
    const builtins = ["editor", "clear", "new"]
  ...
  const pick = (item: CommandEntry) => {
    ...
    if (item.name === "clear" || item.name === "new") {
      props.onNew()
      return
    }
...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "current"
              : model.cost?.input === 0 && provider.id === "kiwii"
                ? "Free"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider = Number(a.providerID !== "kiwii") - Number(b.providerID !== "kiwii")
        if (provider !== 0) {
          return provider
        }
