...
import type * as SDK from "@kiwii/sdk/v2"
...
import { SessionShareTable } from "@kiwii/core/share/sql"
import { ProviderV2 } from "@kiwii/core/provider"
import { ModelV2 } from "@kiwii/core/model"
import { EventV2 } from "@kiwii/core/event"

const disabled = process.env["KIWII_DISABLE_SHARE"] === "true" || process.env["KIWII_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@kiwii/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        ...
        if (!baseUrl) throw new Error("Sharing requires a self-hosted share server configured via enterprise.url")
