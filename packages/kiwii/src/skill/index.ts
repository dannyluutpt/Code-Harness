...
import { SkillPlugin } from "@kiwii/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@kiwii/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@kiwii/core/v1/config/error"
...
const KIWII_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with kiwii. The model's intuition for what an
// kiwii.json should look like is often wrong, and kiwii hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch kiwii's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_KIWII_SKILL_NAME = "customize-kiwii"
const CUSTOMIZE_KIWII_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating kiwii's own configuration: kiwii.json, kiwii.jsonc, files under .kiwii/, or files under ~/.config/kiwii/. Also use when creating or fixing kiwii agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring kiwii itself."
const CUSTOMIZE_KIWII_SKILL_BODY = SkillPlugin.CustomizeKiwiiContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, KIWII_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@kiwii/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_KIWII_SKILL_NAME] = {
          name: CUSTOMIZE_KIWII_SKILL_NAME,
          description: CUSTOMIZE_KIWII_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_KIWII_SKILL_BODY,
        }
