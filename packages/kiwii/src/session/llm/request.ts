...
const USER_AGENT = `kiwii/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const kiwiiProjectID = input.model.providerID.startsWith("kiwii")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("kiwii")
        ? {
            ...(kiwiiProjectID ? { "x-kiwii-project": kiwiiProjectID } : {}),
            "x-kiwii-session": input.sessionID,
            "x-kiwii-request": input.user.id,
            "x-kiwii-client": input.flags.client,
