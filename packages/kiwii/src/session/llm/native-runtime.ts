...
import type { LLMClientShape } from "@kiwii/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("kiwii"))
    return { type: "unsupported", reason: "provider is not openai, kiwii, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @kiwii/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains kiwii-owned. The native runtime only adapts
      // the @kiwii/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
