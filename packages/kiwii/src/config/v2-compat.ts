...
import { NonNegativeInt, PositiveInt } from "@kiwii/core/schema"
import { ConfigAttachmentV1 } from "@kiwii/core/v1/config/attachment"
import { ConfigLSPV1 } from "@kiwii/core/v1/config/lsp"
import { InvalidError } from "@kiwii/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by Kiwii V1. Use V1 "permission" rules or run kiwii2.',
      })),
    })
