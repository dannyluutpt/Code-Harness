...
import { InstallationLocal, InstallationVersion } from "@kiwii/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@kiwii/core/v1/config/console-state"
import { FSUtil } from "@kiwii/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@kiwii/core/util/effect-flock"
...
export class Service extends Context.Service<Service, Interface>()("@kiwii/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.KIWII_CONFIG && !Flag.KIWII_CONFIG_DIR && !Flag.KIWII_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "kiwii.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "kiwii.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://raw.githubusercontent.com/dannyluutpt/Code-Harness/main/schema/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "KIWII_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/kiwii`
        ...
        if (Flag.KIWII_CONFIG) {
          yield* merge(Flag.KIWII_CONFIG, yield* loadFile(Flag.KIWII_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.KIWII_CONFIG })
        }

        if (!Flag.KIWII_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("kiwii", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.KIWII_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from KIWII_CONFIG_DIR", { path: Flag.KIWII_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".kiwii") || dir === Flag.KIWII_CONFIG_DIR) {
            for (const file of ["kiwii.json", "kiwii.jsonc"]) {
          ...
          // Auto-discovered plugins under `.kiwii/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.KIWII_CONFIG_CONTENT) {
          const source = "KIWII_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.KIWII_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from KIWII_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["KIWII_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("KIWII_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.KIWII_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.KIWII_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("KIWII_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
