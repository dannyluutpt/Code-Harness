...
import { isRecord } from "@kiwii/tui/util/record"
...
import { TuiKeybind } from "@kiwii/tui/config/keybind"
...
import { TuiConfig } from "@kiwii/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@kiwii/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old kiwii.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.kiwii`
  // folders between cwd and home, and KIWII_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.KIWII_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit KIWII_TUI_CONFIG override, if set.
  if (Flag.KIWII_TUI_CONFIG) {
    const configFile = Flag.KIWII_TUI_CONFIG
  ...
  // 4. `.kiwii` directories (and KIWII_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".kiwii") || dir === Flag.KIWII_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".kiwii") && dir !== Flag.KIWII_CONFIG_DIR) continue
