...
const TUI_SCHEMA_URL = "https://raw.githubusercontent.com/dannyluutpt/Code-Harness/main/schema/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from kiwii.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const kiwii = await kiwiiFiles(input)
  for (const file of kiwii) {
...
async function kiwiiFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "kiwii"),
    ...(await Filesystem.findUp(["kiwii.json", "kiwii.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "kiwii"))
  }
  if (Flag.KIWII_CONFIG) files.push(Flag.KIWII_CONFIG)
