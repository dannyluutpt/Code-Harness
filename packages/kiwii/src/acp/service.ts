...
import type { AssistantMessage, Message, KiwiiClient, Session, SessionMessageResponse } from "@kiwii/sdk/v2"
...
export const AuthMethodID = "kiwii-login"
...
export class Service extends Context.Service<Service, Interface>()("@kiwii/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `kiwii auth login` in the terminal",
      name: "Login with kiwii",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: KiwiiClient) {
...
function makeUsageService(sdk: KiwiiClient) {
...
async function loadDirectorySnapshot(sdk: KiwiiClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, kiwii provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const kiwiiProvider = providers[ProviderV2.ID.make("kiwii")]
  const kiwiiModel = kiwiiProvider ? Provider.sort(Object.values(kiwiiProvider.models))[0] : undefined
  if (kiwiiProvider && kiwiiModel) return { providerID: kiwiiProvider.id, modelID: kiwiiModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Kiwii service failure", service })
}
