...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for kiwii serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(KiwiiHttpApi)
...
export const PublicApi = KiwiiHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "kiwii",
    version: "1.0.0",
    description: "kiwii api",
