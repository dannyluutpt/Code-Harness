...
// When no web UI bundle is embedded (dev builds), proxy to a local Vite dev server (`bun run dev:web`).
export const UI_UPSTREAM = new URL(process.env["KIWII_WEB_UI_URL"] ?? "http://localhost:3000")
...
export function embeddedUI(disableEmbeddedWebUi: boolean) {
  ...
  return (embeddedUIPromise ??=
    ...
    import("kiwii-web-ui.gen.ts").then((module) => module.default as Record<string, string>).catch(() => null))
}
