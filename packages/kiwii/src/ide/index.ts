...
import { IdeEvent } from "@kiwii/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["KIWII_CALLER"] === "vscode" || process.env["KIWII_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.kiwii"], {
    nothrow: true,
  })
