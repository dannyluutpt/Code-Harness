...
export const DEPRECATED_PLUGIN_PACKAGES = ["kiwii-openai-codex-auth", "kiwii-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, kiwiiVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(kiwiiVersion) || semver.major(kiwiiVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.kiwii
  if (typeof range !== "string") return
  if (!semver.satisfies(kiwiiVersion, range)) {
    throw new Error(`Plugin requires kiwii ${range} but running ${kiwiiVersion}`)
  }
}
