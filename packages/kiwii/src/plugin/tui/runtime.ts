...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@kiwii/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@kiwii/core/effect/app-node-builder"
import { errorData, errorMessage } from "@kiwii/tui/util/error"
...
import { hasTheme, upsertTheme } from "@kiwii/tui/context/theme"
import { Global } from "@kiwii/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@kiwii/core/util/flock"
import { Flag } from "@kiwii/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@kiwii/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@kiwii/core/v1/config/plugin"
import { createCommandShim } from "@kiwii/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@kiwii/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".kiwii"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".kiwii", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".kiwii", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.KIWII_PURE ? [] : pluginOrigins
    if (Flag.KIWII_PURE && pluginOrigins.length) {
    }
