...
Practical reference for a future `packages/server` split after the kiwii
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/kiwii`.
...
Until enough shared service code lives outside `packages/kiwii`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/kiwii`

It should not import `packages/kiwii` services while `packages/kiwii`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on kiwii-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/kiwii` implementation details.
