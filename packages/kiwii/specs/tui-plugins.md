...
- Author package entrypoint is `@kiwii/plugin/tui`.
...
```json
{
  "$schema": "https://raw.githubusercontent.com/dannyluutpt/Code-Harness/main/schema/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/kiwii-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@kiwii/plugin/tui`.
- `@kiwii/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@kiwii/plugin/tui"
...
- The value is a semver range checked against the running Kiwii version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.kiwii` is absent, no check is performed (backward compatible).
...
- `kiwii plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `kiwii plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.kiwii` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.kiwii`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.kiwii`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.kiwii` dir or `KIWII_CONFIG_DIR`, Kiwii installs `@kiwii/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@kiwii/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Kiwii's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Kiwii registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Kiwii pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@kiwii/plugin/tui`.
...
- `message` is required; `title` defaults to `"kiwii"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `kiwii.default`.
...
- Local plugins persist installed themes under the local `.kiwii/themes` area near the plugin config source.
...
- `--pure` / `KIWII_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.kiwii/plugins/tui-smoke.tsx`
- Local vim plugin: `.kiwii/plugins/tui-vim.tsx`
- Local smoke config: `.kiwii/tui.json`
- Local smoke theme: `.kiwii/plugins/smoke-theme.json`
