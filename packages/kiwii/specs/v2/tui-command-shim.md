...
Delete `packages/kiwii/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/kiwii/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/kiwii/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/kiwii`
- TUI plugin loader tests in `packages/kiwii` if runtime plugin API wiring changed
