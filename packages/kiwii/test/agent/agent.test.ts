...
it.instance("plan agent denies edits except .kiwii/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".kiwii/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".kiwii", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.KIWII_TEST_HOME
      process.env.KIWII_TEST_HOME = test.directory
