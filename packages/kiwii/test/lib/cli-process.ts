// Subprocess test harness for the kiwii CLI. Spawns the real binary against
...
//
// Configuration flows through kiwii's built-in test affordances:
//   - KIWII_CONFIG_CONTENT      : provider config inline, no files to find
//   - KIWII_TEST_HOME           : pins os.homedir() → tmpdir
//   - KIWII_DISABLE_PROJECT_CONFIG : skip walking up for kiwii.json
//   - KIWII_PURE                : skip external plugin discovery + install
//   - KIWII_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `kiwii.run` is fully wired. The shape supports adding more
// builders (`kiwii.serve(opts)`, `kiwii.acp(opts)`, `kiwii.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of KiwiiCli.
...
const kiwiiRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(kiwiiRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    KIWII_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    KIWII_CONFIG_CONTENT: configJson,
    KIWII_DISABLE_PROJECT_CONFIG: "1",
    KIWII_PURE: "1",
    KIWII_DISABLE_AUTOUPDATE: "1",
    KIWII_DISABLE_AUTOCOMPACT: "1",
    KIWII_DISABLE_MODELS_FETCH: "1",
    KIWII_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `kiwii run`. New flags should
...
// `kiwii serve` is a long-lived process — it never exits on its own.
...
// `kiwii acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type KiwiiCli = {
  ...
  // Spawn `kiwii serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `kiwii acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("kiwii.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          KIWII_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("kiwii.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("kiwii.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "kiwii server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `kiwii serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("kiwii.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const kiwii: KiwiiCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, kiwii })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `kiwii.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "kiwii") {
...
// every call site. Use this for any test that needs the kiwii CLI fixture.
//
...
// resources (e.g. `kiwii.serve`) without an extra `Effect.scoped` wrapper —
