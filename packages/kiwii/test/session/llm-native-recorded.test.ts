...
import { SessionV1 } from "@kiwii/core/v1/session"
import { ModelsDev } from "@kiwii/core/models-dev"
import { HttpRecorder } from "@kiwii/http-recorder"
import { HttpRecorderInternal } from "@kiwii/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@kiwii/llm"
import { RequestExecutor } from "@kiwii/llm/route"
...
const zenURL = (connection: string) => `https://console.kiwii.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.KIWII_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["kiwii", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("KIWII_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("KIWII_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["kiwii", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["kiwii", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.KIWII_RECORD_CONSOLE_TOKEN && process.env.KIWII_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.kiwii,
        name: "Kiwii Zen",
        env: ["KIWII_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.KIWII_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.KIWII_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.KIWII_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["kiwii", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("KIWII_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("KIWII_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("KIWII_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set KIWII_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the kiwii LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "kiwii.json"),
      JSON.stringify({ $schema: "https://raw.githubusercontent.com/dannyluutpt/Code-Harness/main/schema/config.json", ...scenario.config(model) }),
    ),
  )
