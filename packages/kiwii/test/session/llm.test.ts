...
import { LLMClient, RequestExecutor } from "@kiwii/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const kiwiiFixture = { providerID: "kiwii-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for kiwii providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(kiwiiFixture.providerID),
          ModelV2.ID.make(kiwiiFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(kiwiiFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [kiwiiFixture.providerID],
          provider: {
            [kiwiiFixture.providerID]: {
              name: "Kiwii Test",
