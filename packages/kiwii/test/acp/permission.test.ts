...
import type { Event, KiwiiClient } from "@kiwii/sdk/v2"
...
type PermissionReplyParams = Parameters<KiwiiClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "kiwii-acp-permission-"))
