...
`,
  ...
  const pure = process.env.KIWII_PURE
  const meta = process.env.KIWII_PLUGIN_META_FILE
  process.env.KIWII_PURE = "1"
  process.env.KIWII_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.KIWII_PURE
    } else {
      process.env.KIWII_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.KIWII_PLUGIN_META_FILE
    } else {
      process.env.KIWII_PLUGIN_META_FILE = meta
    }
  }
})
