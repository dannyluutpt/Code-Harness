...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ kiwii }) =>
    Effect.gen(function* () {
      const result = yield* kiwii.spawn(["--replay-limit", "10"])

      kiwii.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ kiwii }) =>
    Effect.gen(function* () {
      const result = yield* kiwii.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ kiwii }) =>
    Effect.gen(function* () {
      const result = yield* kiwii.spawn(["--mini", "--port", "4096"])
