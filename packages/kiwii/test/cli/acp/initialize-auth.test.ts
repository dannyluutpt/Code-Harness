...
describe("kiwii acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ kiwii }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ kiwii }))
        ...
        expect(initialized.agentInfo?.name).toBe("Kiwii")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ kiwii })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("kiwii-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "kiwii-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.KIWII_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("kiwii-login")
