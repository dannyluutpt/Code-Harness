...
// terminal sizes. The default kiwii tui command is excluded —
// `kiwii --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `kiwii --help` prints today;
...
// top-level help on `--help` and exits 1; not a real kiwii command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("kiwii CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const topLevel = yield* kiwii.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* kiwii.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`kiwii ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`kiwii ${argv.join(" ")} --help`)
        }
