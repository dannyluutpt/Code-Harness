...
import { KiwiiClient, type GlobalEvent } from "@kiwii/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<KiwiiClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<KiwiiClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<KiwiiClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<KiwiiClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<KiwiiClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://kiwii.test"),
...
function sdk(
  input: {
    ...
    subscribe?: KiwiiClient["event"]["subscribe"]
    globalEvent?: KiwiiClient["global"]["event"]
    promptAsync?: KiwiiClient["session"]["promptAsync"]
    status?: KiwiiClient["session"]["status"]
    messages?: KiwiiClient["session"]["messages"]
    children?: KiwiiClient["session"]["children"]
    permissions?: KiwiiClient["permission"]["list"]
    questions?: KiwiiClient["question"]["list"]
  } = {},
) {
  const client = new KiwiiClient()

  const subscribe: KiwiiClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: KiwiiClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: KiwiiClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: KiwiiClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: KiwiiClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: KiwiiClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: KiwiiClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: KiwiiClient["question"]["list"] = input.questions ?? (() => ok([]))
