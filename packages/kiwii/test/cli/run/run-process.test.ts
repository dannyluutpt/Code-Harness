// Subprocess integration tests for `kiwii run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `kiwii.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `KIWII_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("kiwii run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kiwii.run("say hi")
        kiwii.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kiwii.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* kiwii.run("think", { extraArgs: ["--thinking"] })
        kiwii.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* kiwii.run("think again")
        kiwii.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kiwii.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kiwii.run("say hi", { format: "json" })
        kiwii.expectExit(result, 0)

        const events = kiwii.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const result = yield* kiwii.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kiwii.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* kiwii.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* kiwii.run("request permission", { permission: { bash: "ask" } })
        kiwii.expectExit(denied, 0)
        ...
        const allowed = yield* kiwii.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        kiwii.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* kiwii.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        kiwii.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, kiwii }) =>
      Effect.gen(function* () {
        ...
        const server = yield* kiwii.serve()

        const result = yield* kiwii.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, kiwii }) =>
      Effect.gen(function* () {
        const result = yield* kiwii.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, kiwii }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* kiwii.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
