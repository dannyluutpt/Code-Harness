...
describe("kiwii read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // KIWII_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["mcp", "list"])
        kiwii.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["providers", "list"])
        kiwii.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["models"])
        kiwii.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["agent", "list"])
        kiwii.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh KIWII_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["session", "list"])
        kiwii.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["stats"])
        kiwii.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under KIWII_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ kiwii }) =>
      Effect.gen(function* () {
        const r = yield* kiwii.spawn(["db", "path"])
        kiwii.expectExit(r, 0, "db path")
