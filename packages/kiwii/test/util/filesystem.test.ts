...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "kiwii.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "kiwii.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kiwii.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kiwii.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["kiwii.json", "kiwii.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "kiwii.json"),
        path.join(project, "kiwii.jsonc"),
      ])
    })
  })
