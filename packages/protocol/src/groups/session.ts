...
import { PromptInput } from "@kiwii/schema/prompt-input"
import { Session } from "@kiwii/schema/session"
import { Project } from "@kiwii/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@kiwii/schema/schema"
...
import { SessionEvent } from "@kiwii/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Kiwii process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Kiwii process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
