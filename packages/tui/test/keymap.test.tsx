...
import { getKiwiiModeStack, KIWII_BASE_MODE, KiwiiKeymapProvider, registerKiwiiKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerKiwiiKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when kiwii mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getKiwiiModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getKiwiiModeStack(keymap).push("autocomplete")
