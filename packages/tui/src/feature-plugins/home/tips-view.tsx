import type { TuiPluginApi } from "@kiwii/plugin/tui"
...
const NO_MODELS_TIP = "Run {highlight}/login{/highlight} to add an AI provider and start coding"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("kiwii.status"),
  ...
  return (
    <box flexDirection="row" maxWidth="100%">
      <text flexShrink={0} style={{ fg: theme.secondary }}>
        ● tip{" "}
      </text>
...
const TIPS: Tip[] = [
  ...
  "Use {highlight}/rewind{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public kiwii.ai link",
  ...
  "Run {highlight}/login{/highlight} to add API keys for 75+ supported LLM providers",
  ...
  "Create {highlight}kiwii.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/kiwii/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.kiwii/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.kiwii/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.kiwii/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.kiwii/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Kiwii from reading sensitive files",
  "Use {highlight}kiwii run{/highlight} for non-interactive scripting",
  "Use {highlight}kiwii --continue{/highlight} to resume the last session",
  "Use {highlight}kiwii run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}kiwii serve{/highlight} for headless API access to Kiwii",
  "Use {highlight}kiwii run --attach{/highlight} to connect to a running server",
  "Run {highlight}kiwii upgrade{/highlight} to update to the latest version",
  "Run {highlight}kiwii auth list{/highlight} to see all configured providers",
  "Run {highlight}kiwii agent create{/highlight} for guided agent creation",
  "Use {highlight}/kiwii{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}kiwii github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/kiwii fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.kiwii/themes/{/highlight} directory",
  ...
  "Run {highlight}kiwii debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/dannyluutpt/Code-Harness{/highlight} in a container",
