import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@kiwii/plugin/tui"
...
import type { useKiwiiKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useKiwiiKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getKiwiiModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getKiwiiModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
