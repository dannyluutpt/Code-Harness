...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@kiwii/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__kiwii_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in kiwii.json to use it.
        </text>
      ),
    })
...
function ApiMethod(props: ApiMethodProps) {
  ...
  return (
    <DialogPrompt
      ...
      placeholder="API key"
      description={() =>
        ({} as Record<string, JSX.Element>)[props.providerID] ?? undefined
      }
      onConfirm={async (value) => {
        if (!value) return
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in kiwii.json to use it.`,
          })
          dialog.clear()
          return
        }
