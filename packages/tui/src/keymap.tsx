...
export const KIWII_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const KIWII_MODE_KEY = "kiwii.mode"

export const KiwiiKeymapProvider = KeymapProvider
export const useKiwiiKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type KiwiiModeStack = ReturnType<typeof createKiwiiModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, KiwiiModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createKiwiiModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(KIWII_MODE_KEY, KIWII_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(KIWII_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(KIWII_MODE_KEY, stack.at(-1)?.mode ?? KIWII_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? KIWII_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(KIWII_MODE_KEY, undefined)
...
export function useKiwiiModeStack() {
  return getKiwiiModeStack(useKiwiiKeymap())
}

export function getKiwiiModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Kiwii mode stack is not registered for this keymap")
  return value
}
...
export function registerKiwiiKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createKiwiiModeStack(keymap)
