...
This is the living design and status document for `@kiwii/codemode` and its existing V2 Kiwii adapter.
...
`packages/kiwii/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
