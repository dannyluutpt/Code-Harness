...
```ts
import { CodeMode, Tool } from "@kiwii/codemode"
...
```ts
import { CodeMode, OpenAPI } from "@kiwii/codemode"
...
const runtime = CodeMode.make({ tools: { kiwii: api.tools } })
...
The catalog-entry budget defaults to 2,000 estimated tokens (characters / 4, the same heuristic Kiwii uses). It applies only to full tool entries shown in the catalog; fixed instructions and namespace summaries are not counted. Override it when constructing a runtime:
...
```ts
import { toolError } from "@kiwii/codemode"
