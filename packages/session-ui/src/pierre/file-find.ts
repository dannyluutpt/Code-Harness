...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("kiwii-find")
  api.delete("kiwii-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("kiwii-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("kiwii-find", new Highlight(...rest))
    return true
  }
