...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@kiwii/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/kiwii`)).toBe("path")
    expect(inlineCodeKind(`@kiwii/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://github.com/dannyluutpt/Code-Harness/blob/main/docs/`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/kiwii`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://kiwii.ai/docs`)).toBeUndefined()
  })
})
