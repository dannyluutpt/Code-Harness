...
> current private `@kiwii/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@kiwii/ai`
...
- Secondary audience: Kiwii and other durable agent runtimes
...
import { LLM } from "@kiwii/ai"
import { OpenAI } from "@kiwii/ai/providers/openai"
...
Kiwii and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@kiwii/ai/promise"
import { OpenAI } from "@kiwii/ai/providers/openai"
...
```ts
import { LLMSchema } from "@kiwii/ai/schema"
...
```ts
import { Provider, Protocol } from "@kiwii/ai/provider"
...
@kiwii/ai/providers/openai
@kiwii/ai/providers/anthropic
@kiwii/ai/providers/google
...
```
...
Kiwii should migrate to `generateTurn` / `streamTurn`, preserving its durable
