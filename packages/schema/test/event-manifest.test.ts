...
describe("public event manifest", () => {
  test("owns the complete public event surface", () => {
    // An inventory guard: these move only when the public event surface really changes.
  ...
  test("uses canonical definitions for current public events", () => {
    ...
    // Order is part of the wire contract. Anchor on the first of the run rather than a fixed
    // offset, which shifts whenever an event lands ahead of it.
    const partDelta = EventManifest.Definitions.indexOf(SessionV1.Event.PartDelta)
    expect(partDelta).toBeGreaterThan(-1)
