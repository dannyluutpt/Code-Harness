...
import { ModelsDevPlugin } from "@kiwii/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.KIWII_MODELS_PATH,
          disabled: Flag.KIWII_DISABLE_MODELS_FETCH,
        }
        Flag.KIWII_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.KIWII_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.KIWII_MODELS_PATH = previous.path
          Flag.KIWII_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
