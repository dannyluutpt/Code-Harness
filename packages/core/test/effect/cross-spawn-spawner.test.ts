...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kiwii-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["KIWII_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { KIWII_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("KIWII_TEST_SHELL=ok")
      }),
    )
