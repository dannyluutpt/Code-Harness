...
import { ConfigProviderPlugin } from "@kiwii/core/config/plugin/provider"
...
describe("ConfigProviderPlugin.Plugin", () => {
  it.effect("keeps configured model variant bodies unchanged", () =>
    Effect.gen(function* () {
      ...
      const providerID = ProviderV2.ID.kiwii
          ...
          Effect.succeed([
            new Config.Document({
              ...
              info: decode({
                providers: {
                  kiwii: {
                    api: { type: "aisdk", package: "@ai-sdk/openai", url: "https://kiwii.test/v1" },
                    models: {
                      "alpha-gpt-next": {
                        variants: [
                          {
