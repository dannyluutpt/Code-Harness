...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@kiwii/acme")).toBe("@kiwii/acme")
    expect(Npm.sanitize("@kiwii/acme@1.0.0")).toBe("@kiwii/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/kiwii/acme.git"
    const expected = win ? "acme@git+https_//github.com/kiwii/acme.git" : spec
