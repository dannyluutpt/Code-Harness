...
import { ToolFailure } from "@kiwii/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@kiwii/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.KIWII_WEBSEARCH_PROVIDER === "exa" || process.env.KIWII_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.KIWII_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("KIWII_EXPERIMENTAL") || truthy("KIWII_ENABLE_EXA") || truthy("KIWII_EXPERIMENTAL_EXA"),
    enableParallel: truthy("KIWII_ENABLE_PARALLEL") || truthy("KIWII_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `kiwii/${InstallationVersion}`,
