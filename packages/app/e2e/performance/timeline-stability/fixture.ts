...
import { SessionStatusEvent } from "@kiwii/schema/session-status-event"
import { SessionV1 } from "@kiwii/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Kiwii/TimelineStability"
...
export const model = { providerID: "kiwii", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("kiwii.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["kiwii"],
    default: { providerID: "kiwii", modelID: "claude-opus-4-6" },
  }
}
