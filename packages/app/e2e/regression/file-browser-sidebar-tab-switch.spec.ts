...
const directory = "C:/Kiwii/FileBrowserSidebar"
...
async function setup(
  ...
  await mockKiwiiServer(page, {
    ...
    provider: {
      ...
      connected: ["kiwii"],
      default: { providerID: "kiwii", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "kiwii.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "kiwii.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "kiwii.window.browser.dat:tabs",
