...
import type { retry } from "@kiwii/core/util/retry"
import type { KiwiiEvent, SessionApi } from "@kiwii/client/promise"
import type { Message, KiwiiClient, Part, Session } from "@kiwii/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as KiwiiEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as KiwiiClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
