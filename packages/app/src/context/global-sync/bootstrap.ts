...
import { NormalizedProviderListResponse } from "@kiwii/session-ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: KiwiiClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("kiwii-test"))
