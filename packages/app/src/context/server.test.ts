  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://kiwii.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://kiwii.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://kiwii.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://kiwii.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://kiwii.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://kiwii.example.com"),
      ),
    ).toEqual({
