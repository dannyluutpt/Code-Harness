import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@kiwii/client/promise"
import { Button } from "@kiwii/ui/button"
import { useDialog } from "@kiwii/ui/context/dialog"
import { Dialog } from "@kiwii/ui/dialog"
import { Icon } from "@kiwii/ui/icon"
import { IconButton } from "@kiwii/ui/icon-button"
import { List, type ListRef } from "@kiwii/ui/list"
import { ProviderIcon } from "@kiwii/ui/provider-icon"
import { Spinner } from "@kiwii/ui/spinner"
import { Tag } from "@kiwii/ui/tag"
import { TextField } from "@kiwii/ui/text-field"
import { ButtonV2 } from "@kiwii/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@kiwii/ui/v2/dialog-v2"
import { TextInputV2 } from "@kiwii/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "kiwii-go") return language.t("dialog.provider.kiwiiGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "kiwii"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.kiwii.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["kiwii", "kiwii-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "kiwii" || provider.id === "kiwii-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "kiwii"
                                ? "dialog.provider.kiwii.tagline"
                                : "dialog.provider.kiwiiGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "kiwii" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "kiwii-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "kiwii"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.kiwiiZen.line1")}</div>
              <div>{language.t("provider.connect.kiwiiZen.line2")}</div>
              <div>
                {language.t("provider.connect.kiwiiZen.visit.prefix")}
                <ExternalLink
                  href="https://github.com/dannyluutpt/Code-Harness"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.kiwiiZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.kiwiiZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "kiwii"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.kiwiiZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.kiwiiZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.kiwiiZen.visit.prefix")}
                <ExternalLink href="https://github.com/dannyluutpt/Code-Harness" tabIndex={-1}>
                  {language.t("provider.connect.kiwiiZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.kiwiiZen.visit.suffix")}
              </div>
