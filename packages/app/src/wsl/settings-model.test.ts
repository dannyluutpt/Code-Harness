...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Kiwii needs attention", () => {
    expect(wslKiwiiAction(undefined)).toBeUndefined()
    expect(
      wslKiwiiAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installKiwii")
    expect(
      wslKiwiiAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/kiwii",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateKiwii")
    expect(
      wslKiwiiAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/kiwii",
  ...
  test("does not report ready when Kiwii is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        kiwiiChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.kiwii/bin/kiwii",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "kiwii is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installKiwii" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-kiwii")
  })
