...
import { wslKiwiiAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.kiwiiChecks[item.config.distro]
          const kiwiiAction = () => wslKiwiiAction(check())
          const busy = () => wsl.data?.job?.kind === "install-kiwii" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={kiwiiAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installKiwii(item.config.distro))}
                    >
