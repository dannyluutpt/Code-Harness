...
export function wslKiwiiAction(check?: WslKiwiiCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installKiwii"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateKiwii"
}
...
export function addServerViewModel(input: {
  ...
  const kiwiiCheck = selectedDistro ? (state?.kiwiiChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.kiwiiChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateKiwii" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.kiwiiMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installKiwii" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingKiwii =
    input.state?.job?.kind === "install-kiwii" && input.state.job.distro === input.selectedDistro
  if (!ready || probingKiwii) {
    return {
      variant: "contrast",
  ...
  if (!addServerKiwiiReady(input.kiwiiCheck)) {
    const update = !!input.kiwiiCheck?.resolvedPath && input.kiwiiCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingKiwii
        ? { key: "wsl.onboarding.updatingKiwii" }
        : update
          ? { key: "wsl.onboarding.updateKiwii" }
          : { key: "wsl.onboarding.installKiwii" },
      disabled: !!input.state?.job || input.adding,
      action: "install-kiwii",
      loading: installingKiwii,
...
function addServerKiwiiReady(check: WslKiwiiCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.kiwiiChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.kiwiiChecks[item.name]) return [`kiwii:${item.name}`]
    return []
  })
