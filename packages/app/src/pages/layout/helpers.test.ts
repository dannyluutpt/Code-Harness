...
import { type Session } from "@kiwii/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("kiwii://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("kiwii://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("kiwii://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("kiwii://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("kiwii://open-project")).toBeUndefined()
    expect(parseDeepLink("kiwii://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "kiwii://open-project?directory=/a",
      "kiwii://other?directory=/b",
      "kiwii://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("kiwii://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("kiwii://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("kiwii://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("kiwii://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "kiwii://new-session?directory=/a",
      "kiwii://open-project?directory=/b",
      "kiwii://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __KIWII__: {
        deepLinks: ["kiwii://open-project?directory=/a"],
      },
    } as unknown as Window & { __KIWII__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["kiwii://open-project?directory=/a"])
