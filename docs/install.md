...
Script tải `kiwii-windows-<arch>.zip` vào `%USERPROFILE%\.kiwii\bin` và thêm vào PATH của người dùng. Ghim phiên bản bằng `$env:VERSION = "0.1.1"` trước khi chạy; `$env:KIWII_NO_MODIFY_PATH = "1"` để tự thêm PATH. Khuyến nghị dùng Windows Terminal; WSL dùng script `install` ở trên.
...
Downloads `kiwii-windows-<arch>.zip` into `%USERPROFILE%\.kiwii\bin` and adds it to the user PATH. Set `$env:VERSION = "0.1.1"` first to pin a version, `$env:KIWII_NO_MODIFY_PATH = "1"` to skip the PATH edit.
