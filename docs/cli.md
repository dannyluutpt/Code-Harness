...
**TUI keys:** `Shift+Tab` permission mode, `Tab` agent, `Ctrl+P` command palette, `Ctrl+X c` compact. Slash commands: `/init`, `/review`, `/commit`, `/pr`, `/compact`, `/sessions`, `/models`, `/agents`, `/mcps`, `/skills`, `/undo`, `/redo`, `/export`, `/help`.
