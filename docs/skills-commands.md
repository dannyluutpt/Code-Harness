...
Lệnh có sẵn: `/init` (tạo `KIWII.md`), `/review [commit|branch|pr]`, `/commit`, `/pr`, `/compact`, `/rewind`, `/redo`, `/export`.
...
Custom slash commands are Markdown files in `.kiwii/command/<name>.md` or `~/.config/kiwii/command/` with optional `description`, `agent`, `model`, `subtask` frontmatter and `$ARGUMENTS`/`$1` placeholders. Built-ins: `/init`, `/review`, `/commit`, `/pr`, `/compact`, `/rewind`, `/redo`, `/export`. Skills are `SKILL.md` folders discovered in `.kiwii/skills/`, `~/.config/kiwii/skills/`, and for Claude Code compatibility `~/.claude/skills/`, `.claude/skills/`, `.agents/skills/`; extra paths via `skills.paths`.
